...
export function LessonScreen({ technique, onComplete, onBack }: LessonScreenProps) {
  ...
  return (
    <div className="mm-lesson-screen">
      ...
      <div className="mm-lesson-content">
      ...
      <div className="mm-lesson-actions">
        {step > 0 && (
          <button className="mm-btn mm-btn-ghost" onClick={() => setStep(s => s - 1)}>← Previous</button>
        )}
        {isLast ? (
          ...
          <button className="mm-btn mm-btn-primary" onClick={() => setStep(s => s + 1)}>
            Next →
          </button>
        )}
      </div>
